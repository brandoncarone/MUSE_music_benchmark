...
#### EMB Extensions: Temporal Localization
In addition to the baseline paper tasks, this repository includes extended, algorithmically verified benchmarks generated by the Extended Music Benchmarks (`emb`) project. These tasks test the model's ability to localize musical transitions over time:
*   **Temporal Meter Shifts:** Detect if and when a time signature shifts (e.g., 4/4 to 3/4).
*   **Temporal Key Modulation:** Detect if and when a harmonic modulation occurs (e.g., C Major to G Major).

The audio stimuli for these extensions are located in the [stimuli/emb_extension/](./stimuli/emb_extension/) directory, and can be evaluated using the custom runners inside [Gemini/](./Gemini/):
*   `Gemini/temporal_meter_Gemini_runner.py`
*   `Gemini/temporal_key_Gemini_runner.py`

For detailed information on the generation method and integration details, see the extension [README.md](./stimuli/emb_extension/README.md).
...
Gemini/                  # Gemini 2.5 Pro / Flash runners (includes temporal_meter & temporal_key extension runners)
...
stimuli/                 # Audio WAV stimuli (canonical root expected by all runners)
  └── emb_extension/     # Extended verified stimuli (meter shifts, key modulations) from the EMB project
fig1_muse_radar.png     
tableA.png             
tableB.png
README.md
